...
cccccc this code is only for CG_comp.f, which should not have band parallelization. 
cccccc thus we will use ug_n_BP as the original ug_n

      call Hpsi_comp(ug_n_BP(1,m1),ugh,ilocal,vr,workr_n,kpt,1,
...
********* the unitary rotation
      ...
      do m=1,mx
       ...
       do m1=1,mx
       c=c+h(m1,m)*ug_n_BP(i,m1)    ! terrible operation
       enddo
      ...
      do m=1,mx
      ug_n_BP(i,m)=ctmp(m)       ! terrible operation
      enddo
200   continue
*********************************
