...
c     Now read in the data for node nnodes
     ...
     & MPI_DOUBLE_COMPLEX,nnodes-1,102,MPI_COMM_K,status,ierr)
        ...
        call mpi_bcast(beta_psi_tmp,nref_tot*mx_n,
     & MPI_DOUBLE_COMPLEX,0,MPI_COMM_B2,ierr)

        endif     ! for the iflag, read
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

ccccc       only one nnodes_b group should write, then all have the same beta
       if(iflag.eq.1.and.icolor_b.eq.0) then     ! write the wavefunction, all icolor_k can get here to write
