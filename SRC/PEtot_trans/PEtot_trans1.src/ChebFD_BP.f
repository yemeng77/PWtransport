...
****************************************
     ...
     &     -dsin(n*dacos(alpha*Ewind(1)+beta)))/(n*pi)
      if(ikernel.eq.-1) then
        gn(n)=(Np-n+1.0d0)/(Np+1.0d0) !Fej\'er kernel
      else if(ikernel.eq.0) then
        gn(n)=((Np-n)*dcos(n*pi/Np)+dsin(n*pi/Np)/dtan(pi/Np))/Np !Jackson kernel
      else if(ikernel.gt.0) then
        gn(n)=(dsin(n*pi/(Np+1.0d0))/(n*pi/(Np+1.0d0)))**ikernel !Lanczos kernel
      else
        if(inode_tot.eq.1) write(6,*) 
...
c       sumdum_m=dconjg(sumdum_m)
       ...
       E_st(1:mst)=E_st(1:mst)*vol
       E_m(1:nblock_band_mx)=E_st(band_dis(1):band_dis(2))
