...
**************************************************
     ...
     &  MPI_MAX,MPI_COMM_K,ierr)
      rho_max=rho_maxtmp
c      rho_cut=rho_max*1.D-2
      if(xgga.lt.1.D-10) then
...
c       if(up+dn.lt.rho_max*1.D-4) then     
c       vxuppbe=0.d0
c       vcuppbe=0.d0
c       vxdnpbe=0.d0
c       vcdnpbe=0.d0
...
c       endif
      ...
      vtot_GGA1=vxuppbe+vcuppbe
      vtot_GGA2=vxdnpbe+vcdnpbe
      E_xc_GGA=(expbe+ecpbe)*(rho(i,1)+rho(i,2)+rhocr(i))
      call UxcCA2(rho(i,1)+rhocr(i)*0.5d0,rho(i,2)+rhocr(i)*0.5d0,
     &    vxc1,vxc2,uxc1,uxc2)
      E_xc_LDA=uxc1*(rho(i,1)+rhocr(i)*0.5d0)+
     &         uxc2*(rho(i,2)+rhocr(i)*0.5d0)
      if(up+dn.gt.0.5*rho_cut) then
        x=(up+dn-0.5*rho_cut)/(0.5*rho_cut)
        fact=dcos(x*pi/2)**2
      vtot(i,1)=vtot(i,1)+vion(i)+vtot_GGA1*(1-fact)+
     &                      vxc1*fact
      vtot(i,2)=vtot(i,2)+vion(i)+vtot_GGA2*(1-fact)+
     &                      vxc2*fact
...
c      if(up+dn.gt.rho_max*1.D-2) then
c      vtot(i,1)=vtot(i,1)+vion(i)+vxuppbe+vcuppbe
c      vtot(i,2)=vtot(i,2)+vion(i)+vxdnpbe+vcdnpbe
c      E_Hxc=E_Hxc+(expbe+ecpbe)*(rho(i,1)+rho(i,2)+rhocr(i))
c      else      ! using LSDA potential, more stable
c      call UxcCA2(rho(i,1)+rhocr(i)*0.5d0,rho(i,2)+rhocr(i)*0.5d0,
c     &    vxc1,vxc2,uxc1,uxc2)
c      vtot(i,1)=vtot(i,1)+vion(i)+vxc1
c      vtot(i,2)=vtot(i,2)+vion(i)+vxc2
c      E_Hxc=E_Hxc+uxc1*(rho(i,1)+rhocr(i)*0.5d0)+
c     &         uxc2*(rho(i,2)+rhocr(i)*0.5d0)
c      endif
