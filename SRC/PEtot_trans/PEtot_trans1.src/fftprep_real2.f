...
cccccc can increase mgz2 significantly
 101   if(mod(nr1*mgz2,nnodes).ne.0) then
       ...
       if(inode_tot.eq.1) then
cccccc this is not a memory problem, instead it is a operation problem (a stupid design).
cccccc by requiring nr1*mgz2 to be divided by nnodes, we later require that each group 
cccccc will do nr1*mgz2/nnodes fft in nr2 direction. If this is more than (1+nr3/2), then 
cccccc it will be stored in the wrong place (no place to store, and it will cause problem. 
cccccc need to be corrected more carefully. 
         write(*,*) 'optimal (and used) value of (pot) mgz2 = ',mgz2
         if(mgz2.gt.(1+nr3/2)) then
         write(*,*) 'load balancing failed, change n1,nnodes_b'
         write(*,*) 'make n1 mode dividable by nnodes_b'
