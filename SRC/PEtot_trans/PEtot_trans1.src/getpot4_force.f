      subroutine getpot4_force(rho,vtot,rhocr,xgga)
******************************************
...
c      if(up.lt.rho_max*1.D-3) then        ! special, make it stable
c       vxuppbe=0.d0
c       vcuppbe=0.d0
c       vxdnpbe=0.d0
c       vcdnpbe=0.d0
...
ccccccccc To make it smoothly merge into LDA, otherwise, this is not stable
      ...
      if(rho(i).gt.0.5*rho_cut) then
        ...
        fact=dcos(x*pi/2)**2
      vtot(i)=vtot_GGA*(1-fact)+vtot_LDA*fact
      else
      vtot(i)=vtot_LDA
      endif
      endif
