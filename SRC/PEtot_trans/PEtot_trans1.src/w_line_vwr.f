     ...
     &  rhoq,rhocq,vqT,ri,amr,lll,nbeta,ido_DOS)
...
cccccccc    and has no 1/rmask for q space implementation
      ...
      if(ierr.ne.0) then
      if(inode.eq.1)
     & write(6,*) "vwr_file ***",vwr_atom,"*** does not exist, stop"
      stop
      endif
...
c      ip_ref=1
c      id_ref=1
ccccc This must be the same as in input.f, otherwise, they will not be consistent
ccccc besides, is_ref in vwr.atom might be useful, i.e, to turn off some reference states in C, O, etc. 
...
**** take the local potential
      ...
      enddo
************************************************
ccccc special, for the ido_DOS, add a Gaussian to vs,vp,vd on top of the local potential
      if(ido_DOS.eq.1) then
      pi=4*datan(1.d0)
      do i=1,nrr
      if(r(i).lt.rcut) then
...
********************************************************
     ...
     &      /2*r(i)**2 
      ...
      if(r(i).lt.rst.and.i.gt.1) then
      stt=stt+vw(i)**2*(r(i+1)-r(i-1))/2*r(i)**2
      endif

      if(ido_DOS.eq.0) then
      vw(i)=vw(i)*amrI
      endif
      ...
      scale=1/dsqrt(dabs(s))
       if(ido_DOS.eq.1) then
cccccccccc assuming ws,wp,wd are properly normalized already, that is true
       scale=1/dsqrt(dabs(stt))
       endif
      endif
      ...
      write(6,*) "iloc,scale", iloc,scale*isNL(ibeta)
      endif
******************************
      if(lll(ibeta).eq.0) then
