...
***  for this point, only add the potential from gauss(r) [(derfc(beta*r)-1)/r], not the delta(r-R)
       if(r.lt.1.D-6) r=1.D-6
       if(rr.gt.cut_rr) then
       sr=sr-1.d0/r
       else
       sr=sr+(derfc(beta*r)-1.d0)/r
       endif
      else
**** for all the other image points, add the potential from gauss(r-R)-delta(r-R) [derfc(beta*r)/r]
...
cccccc the Fortran derfc(x) is good for -infity < x < 10.d0
cccccc! make sure dexp(dz*gk) not overflow, derfc(y) not be zero
      y=dz*beta+gk/beta/2
      if(y.lt.9.5) then    
      sq=sq+dcos(ph)/gk*(
     &  dexp(dz*gk)*derfc(dz*beta+gk/beta/2)+dexp(-dz*gk)*
     &  derfc(-dz*beta+gk/beta/2))
      else
      f22=1.d0     ! f22=exp(y**2)*derfc(y)
      term_iter=1.d0
      do m_iter=1,10    ! serial expansion for exp(y**2)*derfc(y). 
      term_iter=-term_iter*(2*m_iter-1)/(2*y**2)
      f22=f22+term_iter
      enddo
      f22=f22/y/dsqrt(pi)
      sq=sq+dcos(ph)/gk*(f22*dexp(-(dz*beta)**2-(gk/beta/2)**2)
     &  +dexp(-dz*gk)*derfc(-dz*beta+gk/beta/2))
      endif
...
ccccc THIS GAUSSIAN PART LOOKS TOO SMALL !!!   IS THIS REALLY CORRECT ! fact of 10000 !
     ...
     &   dz*beta*derf(dz*beta))       ! this formula is okay numerically 
