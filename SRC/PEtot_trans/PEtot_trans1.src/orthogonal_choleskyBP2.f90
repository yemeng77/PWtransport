SUBROUTINE orthogonal_choleskyBP2(A,SA,n_row,n_col,ng_n,U,iflag)
  ...
  COMPLEX*16, DIMENSION(n_row,n_col) :: A, SA, A_tmp, A_tmp2, AA
...
!$  COMPLEX*16, DIMENSION(mx,mx) :: S, S_t, U
  ...
  INTEGER :: i, j, k, ierr, info, icolor_bb, icol, irow,iflag
...
!!    allocate(S(mx,mx))
  ...
  DO icolor_bb=0, num_group_b-1
     IF (icolor_bb /= icolor_b) THEN     ! this might cause a lot of temperory storage and space
...
!        CALL zgemm('c','n',n_col,n_col,ng_n,one,A,n_row,A,n_row,zero,SS,n_col)
        CALL zgemm('c','n',n_col,n_col,ng_n,one,A,n_row,SA,n_row,zero,SS,n_col)
...
!!              S(irow,icol) = SS(i,j)*vol
     ...
     ELSE
        ...
        CALL mpi_wait(ireq(icolor_bb+1),status,ierr)    ! this can have a problem, because A can still be used!

!         CALL zgemm('c','n',n_col,n_col,ng_n,one,A_tmp,n_row,A,n_row,zero,SS,n_col)
         CALL zgemm('c','n',n_col,n_col,ng_n,one,A_tmp,n_row,SA,n_row,zero,SS,n_col)
...
! should we also rotate SA ?, it is not used here, but perhaps it will be used in other cases, 
! we should have a option to rotate SA as well. 
  ...
  DO icolor_bb=0, num_group_b-1
     IF (icolor_bb /= icolor_b) THEN
        CALL mpi_isend(SA,n_row*n_col,MPI_DOUBLE_COMPLEX, &
             icolor_bb,ikey_b,MPI_COMM_B2,ireq(icolor_bb+1),ierr)
     ENDIF
  ENDDO
  ...
  DO i=1, nblock_band_mx
     ...
     ENDDO
  ENDDO
  CALL zgemm('n','n',ng_n,n_col,n_col,one,SA,n_row,SS,n_col,zero,A_tmp2,n_row)
  AA = AA + A_tmp2
  ...
  SA = AA
   ...
   endif       ! iflag.eq.1: rotate SA
  

 END SUBROUTINE orthogonal_choleskyBP2
