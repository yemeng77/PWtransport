...
ccccc do not calc. force for ia1.eq.ia2, they cancels out anyway for ia1.eq.ia2 image atoms
     ...
     &  derfc(beta*r)/r)/2.d-5     !  The analytical expression for this is available
...
cccccc! make sure dexp(dz*gk) not overflow, derfc(y) not be zero
      y=dz*beta+gk/beta/2
      if(y.lt.9.5) then
      f33=dexp(dz*gk)*derfc(dz*beta+gk/beta/2)
      else
      ...
      f44=dexp(-dz*gk)*derfc(-dz*beta+gk/beta/2)
      ff=f33+f44
      sq=sq+dcos(ph)/gk*ff
...
ccccccccccc  the dz direction force is still not right, it depends on beta !!!
      ...
      ff2=2*0.5d0*derf(dz*beta)     ! 0.5, since later, it will be multiplied by one extra 2
      sqx=sqx+ff2*dz_x0/dztmp      ! these are the dz direction force
     ...
     &   dz*beta*derf(dz*beta))       ! this is the z direction term for total energy
