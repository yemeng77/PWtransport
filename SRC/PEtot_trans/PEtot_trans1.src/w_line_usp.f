...
c        rmesh = r(mesh)
c
ccc        open( unit = iops , file = vwr_atom , status = 'old' ,
ccc     +    form = 'unformatted' )
...
c           nchi = nvales        ! warning, used before defined ! use nvalps ?
       ...
       ncores=0     !  a bit strange, not defined in the original file, snl not used anyway
...
ccccccccccccccccccccccccccccccccccccccccccccc
       implicit double precision (a-h,o-z)
       real*8 vr_tmp(2000),vq_tmp(mnq),qi_tmp(0:mnq+1)
...
202       enddo
          qi_tmp(mnq+1)=mnq*qmx_tmp/(mnq-1.d0)
          qi_tmp(0)=0.d0
cccccccccccccccccccccccccccccccccccccc 

          if(LM0.eq.1.and.inode_tot.eq.1) then
