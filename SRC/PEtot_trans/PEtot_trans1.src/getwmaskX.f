...
************************************************
      ...
      indmtmp(imap)=ii
      r2=r/rcut
      ir=1+r2*200.d0        ! assume equal distance rmask grid

c      f1=(ri(ir+1)-r2)/(ri(ir+1)-ri(ir))
c      f2=(r2-ri(ir))/(ri(ir+1)-ri(ir))
c      rmask=amr(ir)*f1+amr(ir+1)*f2
c      if(ir.eq.1) ir=ir+1
c      if(ir.ge.200) ir=200-1
c      damr0=(amr(ir+1)-amr(ir-1))/(ri(ir+1)-ri(ir-1))
c      damr1=(amr(ir+2)-amr(ir))/(ri(ir+2)-ri(ir))
c      drmask=(damr0*f1+damr1*f2)/rcut
...
cccccccccc  changed, Jan.18,2007, Lin-Wang
      ...
      f2=x+x*(1-x)
      f3=-0.5d0*x*(1-x)       
      rmask=amr(ir)*f1+amr(ir+1)*f2+amr(ir+2)*f3
      drmask=(amr(ir)*(-1.5d0+x)+amr(ir+1)*(2-2*x)+
     &       amr(ir+2)*(-0.5d0+x))/(ri(ir+1)-ri(ir))/rcut
