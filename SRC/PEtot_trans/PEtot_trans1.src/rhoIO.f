...
c      if(icolor.eq.0) then    ! only read from the icolor group
      if(icolor_b.eq.0.and.icolor_k.eq.0) then    ! only read from the icolor group
     
      if (inode==1) then
      open(unit=15,file=fdens_name,form='unformatted',
     & status='old',action='read',iostat=ierr)
      if(ierr.ne.0) then
      write(6,*) "dens_file ***",fdens_name,"*** does not exist, stop"
...
ccccc  We don't need that, ratio, or 1/ratio needs only to be a integer
...
c         call mpi_recv(vr_n_tmp,nri/nnodes,MPI_REAL8,0,100,
c     &        MPI_COMM_K,status,ierr)
         call mpi_recv(vr_n_tmp,nri/nnodes,MPI_REAL8,0,100,
     &        MPI_COMM_B1,status,ierr)
      end if
      call mpi_barrier(MPI_COMM_K,ierr)
...
c      call mpi_bcast(vr_n_tmp,nri/nnodes,MPI_REAL8,0,MPI_COMM_N,ierr)
      call mpi_bcast(vr_n_tmp,nri/nnodes,MPI_REAL8,0,MPI_COMM_B2,ierr)
      call mpi_barrier(MPI_COMM_WORLD,ierr)
      call mpi_bcast(vr_n_tmp,nri/nnodes,MPI_REAL8,0,MPI_COMM_K2,ierr)
...
c      if(icolor.eq.0) then
      if(icolor_b.eq.0.and.icolor_k.eq.0) then
