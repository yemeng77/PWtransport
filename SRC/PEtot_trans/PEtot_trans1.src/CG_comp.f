...
555   continue   ! turning back point for mbad.gt.0 run
      ...
      call orth_comp(ug_n_bp(1,m),ug_n_bp,m-1,2,kpt,
...
************************************************
     ...
     &   1,sug_n(1,m),sumdum,iislda)
        call orth_comp(ug_n_bp(1,m),ug_n_bp,0,1,kpt,
...
****      = < ugh(i) | ugh(i) >
************************************************
**** orthogonalization to ug_n_bp(i,m) is done here outside, instead of 
**** inside orth_com, since we want E_ug, and err 
      ...
      do i=1,ng_n
      s=s+dreal(ugh(i)*dconjg(ug_n_bp(i,m)))
      enddo
      ...
      if(ipsp_all.eq.1) then
      do i=1,ng_n
      err=err+cdabs(ugh(i)-s*ug_n_bp(i,m))**2
      pg(i)=ugh(i)-s*ug_n_bp(i,m)
      enddo
      else
      do i=1,ng_n
...
************************************************
      call orth_comp(pg,ug_n_bp,m-1,2,kpt,
...
**********************************************
************************************************
      call orth_comp(pg,ug_n_bp,m,2,kpt,
     & sug_n,spg,ipsp_all,0,1,fnorm)
...
**********************************************
     ...
     & sumdum2,iislda)
      ...
      call orth_comp(pg,ug_n_bp,0,1,kpt,       ! now we can normalize pg
...
**********************************************
      do i=1,ng_n
      ug_n_bp(i,m)=ug_n_bp(i,m)*cos_th+pg(i)*sin_th
      enddo
