...
*************************************************************
     ...
     &  MPI_MAX,MPI_COMM_K,ierr)
      rho_max=rho_maxtmp
ccc      rho_cut=rho_max*1.D-2
      if(xgga.le.1.D-10) then
      rho_cut=rho_max*1.D-10
      else
      rho_cut=rho_max*xgga
      endif
...
c      if(up.lt.rho_max*1.D-4) then        ! special, make it stable
c       vxuppbe=0.d0
c       vcuppbe=0.d0
c       vxdnpbe=0.d0
c       vcdnpbe=0.d0
...
ccccccccc To make it smoothly merge into LDA, otherwise, this is not stable
ccccccccc This does not work well, the energy is not variational.  
ccccccccc We should require Ecut2 => 4*Ecut for GGA calculations
cccccccc This is effectively not used because rho_cut is very small
cccccccc Besides, rho could be negative if it goes through convert_SLvr.f
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

      if(rho(i).gt.rho_cut) then        
      vtot(i)=vtot(i)+vion(i)+vxuppbe+vcuppbe
      E_Hxc=E_Hxc+(expbe+ecpbe)*(rho(i)+rhocr(i))
      else
      vtot_GGA=vxuppbe+vcuppbe
      E_xc_GGA=(expbe+ecpbe)*(rho(i)+rhocr(i))
      vtot_LDA=UxcCA(rho(i)+rhocr(i),uxc2)
      E_xc_LDA=uxc2*(rho(i)+rhocr(i))
      if(rho(i).gt.0.5*rho_cut) then       
        x=(rho(i)-0.5*rho_cut)/(0.5*rho_cut)
        fact=dcos(x*pi/2)**2
      vtot(i)=vtot(i)+vion(i)+vtot_GGA*(1-fact)+
     &                      vtot_LDA*fact
      E_Hxc=E_Hxc+E_xc_GGA*(1-fact)+E_xc_LDA*fact
      else
      vtot(i)=vtot(i)+vion(i)+vtot_LDA
      E_Hxc=E_Hxc+E_xc_LDA
      endif
      endif

cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c      if(rho(i).gt.rho_max*1.D-2) then
c      vtot(i)=vtot(i)+vion(i)+vxuppbe+vcuppbe
c      E_Hxc=E_Hxc+(expbe+ecpbe)*(rho(i)+rhocr(i))
c      else        ! use LDA formula
c      vtot(i)=vtot(i)+vion(i)+UxcCA(rho(i)+rhocr(i),uxc2)
c      E_Hxc=E_Hxc+uxc2*(rho(i)+rhocr(i))
c      endif
