...
cccccccc Lin-Wang Wang 
ccccccc for cylindrical cutoff, we might need much larger qmx to cover the all the G points
ccccccc inside the cylinder 
       qmx=dsqrt(2*Ecut2)*1.2d0     ! old option
cc      qmx=dsqrt(2*Ecut2)*3.0d0        ! new one
...
********************************************************
     ...
     &      /2*r(i)**2 
      ...
      if(isp.eq.iloc.or.abs(s/s_w).lt.1.D-10) then
      scale=0.d0
      else
...
******************************************
      ...
      rho_atomIr(i,ia)=
     & f1*(occ_s(ia)*ws(i1m)**2+occ_p(ia)*wp(i1m)**2+
     & occ_d(ia)*wd(i1m)**2)/(4*pi)+
     & f2*(occ_s(ia)*ws(i1)**2+occ_p(ia)*wp(i1)**2+
     & occ_d(ia)*wd(i1)**2)/(4*pi)
       goto 3302
       endif
       enddo
3302   continue
