!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! File zminresqlpModule.f90
!
! MINRESQLP solves Hermitian systems Ax = b or min ||Ax - b||_2,
...
!
! 22 Aug 2012: First version of zminresqlpModule.f90.
! 16 Sep 2012: dot_product replaces zdotc.
!              symortho mimics crotg.
! 30 Oct 2012: shift declared real(dp), not complex(dp).
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

module zminresqlpModule
   implicit none
   ...
   real(dp), parameter :: zero = 0.0_dp, one = 1.0_dp, eps = epsilon(zero)
   real(dp), parameter :: realmin = tiny(one), realmax = huge(one)
   complex(dp), parameter :: zzero = cmplx(0.0_dp,0.0_dp,dp), zone = cmplx(1.0_dp,0.0_dp,dp)
   integer, parameter :: prcsn   = precision(zero)
   logical :: debug  = .false.

   integer :: inode

   private                                       ! sets default for module
   public   :: ZMINRESQLP, ZSYMORTHO

contains

  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

  subroutine ZMINRESQLP( n, Aprod, b, itnlim, vol, shift, Msolve, nout,   &
                        disable, rtol, maxxnorm, trancond, Acondlim,      &
                        x, istop, itn, rnorm, Arnorm, xnorm, Anorm, Acond )
    ! Inputs
    integer(ip), intent(in)            :: n
    complex(dp), intent(in)            :: b(n)
    integer(ip), intent(in)            :: itnlim
    ...
    !------------------------------------------------------------------
    !
    ! ZMINRESQLP  is designed to solve the system of linear equations
    !
    !                Ax = b
    !
    ...
    !         min || Ax - b ||_2,
    !
    !     where A is an n by n Hermitian matrix and b is a given complex vector.
    ...
    !        number of iterations as zMINRESQLP but less work per iteration.
    !        But if a low-accuracy solution is adequate, MINRESQLP will terminate sooner.
    !
    !     2. If A is indefinite but Ax = b is known to have a solution
    !        (e.g. if A is nonsingular), SYMMLQ might be preferred,
    !        since it requires roughly the same number of iterations as zMINRESQLP
    !        but slightly less work per iteration.
    !
    !     3. If A is indefinite and well-conditioned, and Ax = b have a
    ...
    !
    !
    !     More generally, zMINRESQLP is designed to solve the system
    !
    !                (A - shift*I) x = b
    !     or
    !         min || (A - shift*I) x - b ||_2,
    !
    !     where  shift  is a specified real scalar value.  Again, the matrix
    ...
    !     to use SYMMLQ.
    !
    !     In this documentation, ' denotes the conjugate transpose
    ...
    !     eps is computed by zMINRESQLP.  A typical value is eps = 2.22d-16
    ...
    !                        used to check if A is Hermitian.
    !
    ...
    !
    !     shift   input      (real) Should be zero if the system Ax = b is to be
    ...
    !                          open(nout, file='zMINRESQLP.txt', status='unknown')
    !                        If nout > 0, a summary of the iterations
    !                        will be printed on unit nout. If the file associated with
    !                        nout is not opened properly, results will be written
    !                        to 'zMINRESQLP_tmp.txt'.
    !                        (Avoid 0, 5, 6 because by convention stderr=0, 
    !                        stdin=5, stdout=6.)
    !
    !     itnlim  input      An upper limit on the number of iterations. Default to 4n.
    !
    !     rtol    input      A user-specified tolerance.  zMINRESQLP terminates
    ...
    !
    !                        If shift = 0 and there is no preconditioner, zMINRESQLP
    ...
    !                        If disable = .true., then a null vector was obtained,
    !                        given rtol.
    !
    ...
    !                        This is sqrt( x'Mx ).  If Moslve is absent,
    ...
    !------------------------------------------------------------------
    !
    !     MINRESQLP is an implementation of a Hermitian version of the
    !     algorithm described in the following references:
    !
    !     Sou-Cheng Choi,
    ...
    !     singular symmetric systems, 33, 4, 1810-1836, SIAM Journal of
    ...
    !     ACM Transactions on Mathematical Software, accepted Jan 2012.
    !
    ...
    !     16 Sep 2012: Version 20: dot_product replaces zdotc.
    !                  x(1) printing needs x1last to be updated.
    ...
    intrinsic :: abs, dot_product, sqrt, real, aimag, present, floor, log10
    ...
    logical      :: prnt, done, lastiter, likeLS, connected, named_file
    ...
    character(len=*), parameter :: msg(1:15) =                                &
     ...
     character(len=*), parameter :: zdebugStr2 = "(7(a, i0, a, e12.3, a, e12.3, a))"

     character(len=*), parameter :: headerStr =                    &
        "(// 1p,    a, 4x, 'Solution of Hermitian   Ax = b'"    // &
        "  / ' n        =', i7, 6x,  'checkA   =', l4, 10x,"    // &
        "     'precon   =', l4                           "      // &
     ...
     character(len=*), parameter :: itnStr = "(1p, i8, 2e19.10, 7e10.2, a)"
    ...
    if (present(nout) .and. inode == 1) then
       nout_ = nout
    ...
    if (.not. connected .and. inode == 1) then
        write(*,*) 'nout_ = ', nout_
        filename = "zMINRESQLP.txt"
    ...
    flag0    = 0
    istop_   = flag0
    beta1    = real(zdotc(n, b, 1, b, 1))
    call global_sumr(beta1)
    beta1    = sqrt(beta1 * vol)
    ieps     = 0.1_dp/eps
    itn_     = 0
    QLPiter  = 0
    xnorm_   = zero
    ...
    if (inode == 1 .and. nout_ > 0) then
       write(nout_, headerStr) enter, n, checkA_, precon_, itnlim_, rtol_, &
    ...
    r_tmp = real(zdotc(n, y, 1, y, 1), dp)
    call global_sumr(r_tmp)
    r_tmp = sqrt(r_tmp * vol)

    if (beta1 < zero .and. r_tmp > eps) then     ! M must be indefinite.
       istop_ = 11
    end if
    ...
    ! s, t are complex.   t1, t2, z are real.
    !------------------------------------------------------------------
    if (checkA_  .and.  precon_) then
       ...
       if (debug .and. inode == 1) then
         write(*,*) 'Check that M is Hermitian:'
         write(*,*) '   s = ', s
         write(*,*) '   t = ', t
         write(*,*) '   z = ', z
         write(*,*) 'epsa = ', epsa
    ...
    if (checkA_) then
       ...
       epsa   = (abs(s) + eps) * eps**0.33333_dp
       if (debug .and. inode == 1) then
         write(*,*) 'Check that A is Hermitian:'
         write(*,*) '   s = ', s
         write(*,*) '   t = ', t
         write(*,*) '   z = ', z
         write(*,*) 'epsa = ', epsa
    ...
    if (inode == 1 .and. nout_ > 0) write(nout_, tableHeaderStr)
    ...
    do while (istop_ <= flag0)
       ...
       alfa = real(zdotc(n,v,1,y,1), dp)      ! alphak
       call global_sumr(alfa)
       alfa = alfa * vol
       y    = y + (- alfa/beta) * r2
       r1   = r2
       r2   = y
       ...
       if ( .not. precon_ ) then
          r_tmp =  real(zdotc(n,y,1,y,1), dp)      ! alphak
          call global_sumr(r_tmp)
          r_tmp = r_tmp * vol 
          betan = sqrt(r_tmp)                  ! betan = ||y||_2
       else
          call Msolve( n, r2, y )
          betan = real(zdotc(n,r2,1,y,1), dp)     ! betan = betak+1^2
          call global_sumr(betan)
          betan = betan * vol
          r_tmp =  real(zdotc(n,y,1,y,1), dp)      ! alphak
          call global_sumr(r_tmp)
          r_tmp = sqrt(r_tmp * vol)
          if (betan > zero) then
             betan = sqrt(betan)
          elseif ( r_tmp > eps ) then    ! M must be indefinite.
             istop_ = 11
             exit
          end if
       end if
       ...
       if (itn_ == 1) then
          vec2(1)  = alfa
          vec2(2)  = betan
          pnorm    = dznrm2(2, vec2, 1)
       else
          vec3(1) = beta
          vec3(2) = alfa
          vec3(3) = betan
          pnorm   = dznrm2(3, vec3, 1)
       end if
       ...
       if (debug .and. inode == 1) then
          ...
          write(*,ddebugStr1) 'c_',    itn_-1, ' = ', cs
          write(*,zdebugStr2) 's_',    itn_-1, ' = ', real(sn),    ' + I * ', aimag(sn)
          write(*,zdebugStr2) 'dlta_', itn_,   ' = ', real(dlta),  ' + I * ', aimag(dlta),  ', ', &
                              'gbar_', itn_,   ' = ', real(gbar),  ' + I * ', aimag(gbar),  ', ', &
                              'epln_', itn_+1, ' = ', real(eplnn), ' + I * ', aimag(eplnn), ', ', &
                              'dbar_', itn_+1, ' = ', real(dltan), ' + I * ', aimag(dltan)
       end if
       ...
       gamal2 = gamal
       gamal  = gama
       call ZSYMORTHO(gbar, cmplx(betan,zero,dp), cs, sn, gama)
       gama_tmp = gama;
       taul2  = taul
       taul   = tau
       tau    = cs * phi
       phi    = sn * phi                     ! phik
       Axnorm = sqrt( Axnorm**2 + abs(tau)**2 );

       if (debug .and. inode == 1) then
          write(*,*)
          write(*,*) 'Compute the current left reflection Q_{', itn_, ',', itn_+1, '}'
          write(*,ddebugStr1) 'c_',    itn_, ' = ', cs
          write(*,zdebugStr2) 's_',    itn_, ' = ', real(sn),   ' + I * ', aimag(sn)
          write(*,zdebugStr2) 'tau_',  itn_, ' = ', real(tau),  ' + I * ', aimag(tau), ', ', &
                              'phi_',  itn_, ' = ', real(phi),  ' + I * ', aimag(phi), ', ', &
                              'gama_', itn_, ' = ', real(gama), ' + I * ', aimag(gama)
       end if
       ...
       if (itn_ > 2) then
          ...
          if (debug .and. inode == 1) then
             ...
             write(*,ddebugStr1) 'cr2_',    itn_,   ' = ', cr2
             write(*,zdebugStr2) 'sr2_',    itn_,   ' = ', real(sr2),    ' + I * ', aimag(sr2)
             write(*,zdebugStr2) 'gamal2_', itn_,   ' = ', real(gamal2), ' + I * ', aimag(gamal2), ', ', &
                                 'gamal_',  itn_,   ' = ', real(gamal),  ' + I * ', aimag(gamal),  ', ', &
                                 'gama_',   itn_,   ' = ', real(gama),   ' + I * ', aimag(gama)
             write(*,zdebugStr2) 'dlta_',   itn_,   ' = ', real(dlta),   ' + I * ', aimag(dlta),   ', ', &
                                 'vepln_',  itn_-1, ' = ', real(veplnl), ' + I * ', aimag(veplnl)
           end if
       end if
       ...
       if (itn_ > 1) then
          call ZSYMORTHO(gamal, dlta, cr1, sr1, gamal_tmp)
          gamal     = gamal_tmp
          vepln     = sr1 * gama
          gama      = -cr1 * gama
          ...
          if (debug .and. inode == 1) then
             write(*,*)
             write(*,*) 'Apply the current right reflectionf  P_{', itn_-1, ',', itn_, '}'
             write(*,ddebugStr1) 'cr1_',   itn_,   ' = ', cr1
             write(*,zdebugStr2) 'sr1_' ,  itn_,   ' = ', real(sr1),    ' + I * ', aimag(sr1)
             write(*,zdebugStr2) 'gama_',  itn_-2, ' = ', real(gamal2), ' + I * ', aimag(gamal2), ', ', &
                                 'gama_',  itn_-1, ' = ', real(gamal),  ' + I * ', aimag(gamal),  ', ', &
                                 'gama_',  itn_,   ' = ', real(gama),   ' + I * ', aimag(gama)
             write(*,zdebugStr2) 'dlta_',  itn_,   ' = ', real(dlta),   ' + I * ', aimag(dlta),   ', ', &
                                 'vepln_', itn_-1, ' = ', real(veplnl), ' + I * ', aimag(veplnl), ', ', &
                                 'eta_',   itn_,   ' = ', real(eta),    ' + I * ', aimag(eta)
          end if
       end if
       ...
       if (itn_ > 2) then
          ...
          if (debug .and. inode == 1) then
             write(*,zdebugStr2) 'tau_',   itn_-2, ' = ', real(taul2),   ' + I * ', aimag(taul2),   ', ', &
                                 'eta_',   itn_-2, ' = ', real(etal2),   ' + I * ', aimag(etal2),   ', ', &
                                 'vepln_', itn_-2, ' = ', real(veplnl2), ' + I * ', aimag(veplnl2), ', ', &
                                 'gama_',  itn_-2, ' = ', real(gamal2),  ' + I * ', aimag(gamal2)
          end if
       end if
       if (itn_ > 1) then
          ul  = ( taul  - etal  * ul3 - veplnl  * ul2) / gamal
          if (debug .and. inode == 1) then
             write(*,zdebugStr2) 'tau_',   itn_-1, ' = ', real(taul),   ' + I * ', aimag(taul),   ', ', &
                                 'eta_',   itn_-1, ' = ', real(etal),   ' + I * ', aimag(etal),   ', ', &
                                 'vepln_', itn_-1, ' = ', real(veplnl), ' + I * ', aimag(veplnl), ', ', &
                                 'gamal_', itn_-1, ' = ', real(gamal),  ' + I * ', aimag(gamal)
          end if
       end if
       ...
       vec3(2) = ul2
       vec3(3) = ul
       xnorm_tmp = dznrm2(3, vec3, 1)      !norm([xl2norm ul2 ul]);
       if (abs(gama) > zero  .and.  xnorm_tmp < maxxnorm_) then
          if (debug .and. inode == 1) then
             write(*,zdebugStr2) 'tau_',   itn_, ' = ', real(tau),   ' + I * ', aimag(tau) ,   ', ', &
                                 'eta_',   itn_, ' = ', real(eta),   ' + I * ', aimag(eta) ,   ', ', &
                                 'vepln_', itn_, ' = ', real(vepln), ' + I * ', aimag(vepln)
             write(*,zdebugStr2) 'gama_',  itn_, ' = ', real(gama),  ' + I * ', aimag(gama)
          end if
          u       = (tau - eta*ul2 - vepln*ul) / gama
          likeLS  = relAresl < relresl
          vec2(1) = xnorm_tmp
          vec2(2) = u
          if (likeLS  .and.  dznrm2(2, vec2, 1) > maxxnorm_) then
             u      = zero
             istop_ = 12
          end if
       else
       ...
       vec2(1)   = xl2norm
       vec2(2)   = ul2
       xl2norm   = dznrm2(2, vec2, 1)
       vec3(1)   = xl2norm
       vec3(2)   = ul
       vec3(3)   = u
       xnorm_    = dznrm2(3, vec3, 1)

       if (Acond_ < trancond_  .and.  istop_ == flag0  .and.  QLPiter == 0) then !! MINRES updates
          wl2  = wl
          wl   = w
          if (abs(gama_tmp) > zero) then
             s = one / gama_tmp
          ...
          if (debug .and. inode == 1) then
             ...
             write(*,zdebugStr2) 'gama_tmp_', itn_, ' = ', real(gama_tmp), ' + I * ', aimag(gama_tmp), ', ', &
                                 'tau_',      itn_, ' = ', real(tau),      ' + I * ', aimag(tau)     , ', ', &
                                 'epln_',     itn_, ' = ', real(epln),     ' + I * ', aimag(epln)    , ', ', &
                                 'dlta_QLP_', itn_, ' = ', real(dlta_QLP), ' + I * ', aimag(dlta_QLP)
             write(*,ddebugStr1) 'v_', itn_ , ' = ', (v(j), j=1,nprint)
             write(*,ddebugStr1) 'w_', itn_ , ' = ', (w(j), j=1,nprint)
          end if
       ...
       if (debug .and. inode == 1) then
          ...
          write(*,zdebugStr2) 'u_', itn_-2, ' = ', real(ul2), ' + I * ', aimag(ul2), ', ', &
                              'u_', itn_-1, ' = ', real(ul),  ' + I * ', aimag(ul),  ', ', &
                              'u_', itn_,   ' = ', real(u),   ' + I * ', aimag(u)
       ...
       if (debug .and. inode == 1) then
          ...
          write(*,zdebugStr2) 'gama_', itn_-1, ' = ', real(gamal), ' + I * ', aimag(gamal), ', ', &
                              'epln_', itn_+1, ' = ', real(eplnn), ' + I * ', aimag(eplnn)
       end if

       gamal_tmp = gamal
       call ZSYMORTHO(gamal_tmp, eplnn, cr2, sr2, gamal)

       if (debug .and. inode == 1) then
          write(*,ddebugStr2) 'cr2_',  itn_+1, ' = ', cr2
          write(*,zdebugStr2) 'sr2_',  itn_+1, ' = ', real(sr2),   ' + I * ', aimag(sr2), ', ', &
                              'gama_', itn_-1, ' = ', real(gamal), ' + I * ', aimag(gamal)
       end if
       ...
       if (debug .and. inode == 1) then
          ...
          write(*,zdebugStr2) 'gama_QLP_', itn_-1, ' = ', real(gamal_QLP), ' + I * ', aimag(gamal_QLP), ', ', &
                              'vepln_QLP_',itn_,   ' = ', real(vepln_QLP), ' + I * ', aimag(vepln_QLP), ', ', &
                              'gama_QLP_', itn_,   ' = ', real(gama_QLP),  ' + I * ', aimag(gama_QLP)
          write(*,zdebugStr2) 'u_QLP_', itn_-2,    ' = ', real(ul2_QLP),   ' + I * ', aimag(ul2_QLP)  , ', ', &
                              'u_QLP_', itn_-1,    ' = ', real(ul_QLP),    ' + I * ', aimag(ul_QLP)   , ', ', &
                              'u_QLP_', itn_,      ' = ', real(u_QLP),     ' + I * ', aimag(u_QLP)
       end if
       ...
       abs_gama = abs(gama)
       Anorml   = Anorm_
       Anorm_   = max(Anorm_, pnorm, abs(gamal), abs_gama)
       if (itn_ == 1) then
!         gmin  = gama    ! Should be abs_gama?
          gmin  = abs_gama
          gminl = gmin
       else if (itn_ > 1) then
          gminl2  = gminl
       ...
       rnorml   = rnorm_
       relresl  = relres
       if (istop_ /= 14) rnorm_ = abs(phi)
       relres   = rnorm_ / (Anorm_ * xnorm_ + beta1)
       vec2(1)  = gbar
       vec2(2)  = dltan
       rootl    = dznrm2(2, vec2, 1)
       ...
       if (istop_ /= flag0) then
          ...
          wl2 = wl2 - shift_*r1
          Arnorm_ = real(zdotc(n, wl2, 1, wl2, 1), dp)
          call global_sumr(Arnorm_)
          Arnorm_ = sqrt(Arnorm_ * vol)
       ...
       if (nout_ > 0) then
          ...
          if ( prnt .and. inode == 1) then
             write(nout_, itnStr) itn_-1, real(x1last), aimag(x1last), xnorml, &
             rnorml, Arnorml, relresl, relAresl, Anorml, Acondl, QLPstr
             if (mod(itn_,10) == 0) write(nout_, "(1x)")
          end if
          if ( inode == 1) then
            write(6, itnStr) itn_-1, real(x1last), aimag(x1last), xnorml, &
            rnorml, Arnorml, relresl, relAresl, Anorml, Acondl, QLPstr
         end if
       end if
    ...
    if ( inode == 1) then
       write(6, itnStr) itn_, x(1), xnorm_, &
             rnorm_, Arnorm_, relres, relAres, Anorm_, Acond_, QLPstr
    end if
    ...
    return
  end subroutine ZMINRESQLP

  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

  subroutine ZSYMORTHO(a, b, c, s, r)

    complex(dp), intent(in)   :: a, b
    real(dp),    intent(out)  :: c
    complex(dp), intent(out)  :: s, r

    !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! ZSYMORTHO: Stable Householder reflection
    !
    !  USAGE:
    !     ZSYMORTHO(a, b, c, s, r)
    !
    ...
    !           NB: c is REAL
    !    s      sine(theta)
    !    r      reflected vector of [a; b] in complex plane
    !
    !  DESCRIPTION:
    !     Stable Householder reflection that gives c and s such that
    !        [ c  s ][a] = [r],
    !        [ s -c ][b]   [0]
    !     where abs(r) = two norm of vector [a, b],
    !        c = a / r,
    !        s = b / r.
    ...
    !
    !  REFERENCES:
    !    Subroutine CROTG from http://www.netlib.org/blas/crotg.f.
    !
    !  MODIFICATION HISTORY:
    !    05/27/2011: Created this file from Matlab SymGivens2.m
    !    09/15/2012: Revised to follow CROTG.
    !                Need to check if SymGivens2.m works correctly
    !                when a and b are complex.
    !
    !  KNOWN BUGS:
    ...
    if (abs_b <= realmin) then
       ...
       if (debug .and. inode == 1) then
          write(*,*) 'a = 0',  'c = ',  c, ', s = ',  s, ', r = ',  r
       end if

    else if (abs_b > abs_a) then
       t = abs_a / abs_b
       c = one / sqrt(one + t**2)
       s = c * conjg((b / abs_b) / (a / abs_a))
       c = c * t
       r = b / conjg(s)

       if (debug .and. inode == 1) then
          write(*,*)  't = ',  t, 'c = ',  c, ', s = ',  s, ', r = ',  r
       end if

    else
       t = abs_b / abs_a
       c = one / sqrt(one + t**2)
       s = c * t * conjg( (b / abs_b) / (a / abs_a));
       r = a / c
