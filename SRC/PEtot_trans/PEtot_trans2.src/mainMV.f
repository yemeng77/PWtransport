...
********************************************
       ...
       real*8,allocatable,dimension(:) :: rhonew1_n,rhonew2_n
...
c       nnodes = n$pes
       ...
       call input(AL,ilocal,tolug,tolE,mxlow,
     ...
     &  num_mov,tolforce,imov_at,dtstart,dd_limit,
     &  idens_out,kpt_dens,ispin_dens,iw_i,iw_f,fdens_out,
     &  f_xatom,ivext_in,fvext_in,ivext_dir,xvext_c,dv_mix,
     &  nite_mix,dv_jump_init,file_dV,imv_cont,
...
ccccccccccc
       ...
       write(22,305) Ecut*2,Ecut2*2,Smth
       write(22,306) totNel,mx,mxlow,tolug,tolE
       write(6,306) totNel,mx,mxlow,tolug,tolE
       write(22,307) ilocal,rcut,ntype
       write(22,308) nkpt,nrot,num_mov,dtstart,dd_limit
       write(22,*) "*********************"
...
305    format("  Ecut=",f7.2,  "    Ecut2=",f7.2,"      Smth=",f7.2)
306    format("totNel=",f7.2,  "       mx=",i7,  "     mxlow=",i7,
     &   "   tolug=",E7.1, "   tolE=",E7.1)
...
**********************************
     ...
     &        AL(3,1)*(AL(1,2)*AL(2,3)-AL(2,2)*AL(1,3))
       ...
       call gen_G2_real()
       call fftprep_real2(n1,n2,n3)


       if(nrot.gt.1) then
       allocate(ig_star(mr_n))       ! defined in data.f
       allocate(ig_local(mr_n))      ! not, mr_n/2, since -k might be in it.
       allocate(ig_lenstar(mr_n))
...
ccccccc special, for wavefunction recovery
cc       call ugIO(ug_n,kpt,2,0,iislda)
cc       call mpi_barrier(MPI_COMM_WORLD,ierr)
cc       if(inode.eq.1) write(6,*) "read in the wave from ugiofile"
cccccc  end special
       ...
       enddo
*********************************************************
       if(idens_out.eq.1)  then     ! output charge, then stop

       if((kpt_dens.gt.nkpt.or.ispin_dens.gt.islda.or.
     &  iw_i.gt.iw_f.or.iw_f.gt.mx).and.inode.eq.1) then
       write(6,*) "something wrong for output charge density"
       write(6,*) "kpt_dens.gt.nkpt.or.ispin_dens.gt.islda"
       write(6,*) "or.iw_i.gt.iw_f.or.iw_f.gt.mx, stop"
       write(6,*) "kpt_dens,nkpt,ispin_dens,islda,iw_i,iw_f,mx", 
     &  kpt_dens,nkpt,ispin_dens,islda,iw_i,iw_f,mx 
       call mpi_abort(MPI_COMM_WORLD,ierr)
       endif

       call gen_G_comp(kpt_dens,0) 
       call fftprep_comp(n1,n2,n3)
       call ugIO(ug_n,kpt_dens,2,0,ispin_dens) 
       call dens_outWG(AL,workr_n,iw_i,iw_f,fdens_out,kpt_dens)
       goto 5000  ! clean up files
       endif
*********************************************************
       if(idens_out.eq.3)  then     ! output charge, then stop, for all kpoints
     ...
     &  kpt_dens,nkpt,ispin_dens,islda,iw_i,iw_f,mx 
       ...
       fdens_out=trim(file_tmp)//char(48+ikpt_100)//char(48+ikpt_10)//
     &   char(48+ikpt_1)
       call dens_outWG(AL,workr_n,iw_i,iw_f,fdens_out,kpt_dens)
c For LDOS calculation 
c       call dens_outWGsp(AL,workr_n,iw_i,iw_f,fdens_out,kpt_dens)
       enddo
       goto 5000  ! clean up files
       endif
*********************************************************
	  ...
	  Ealphat=Ealphat*ch/vol
       ...
       call getNLsign(iatom,ntype,is_ref,ip_ref,id_ref)


      
       if(irho_in.eq.1) then
       do iislda=1,islda
       call rhoIO(AL,rho_n(1,iislda),mr_n,2,frho_in(iislda))     
       enddo
       ido_rho=0
       else
       ido_rho=1   
       ...
       if(ivr_in.eq.1) then
       do iislda=1,islda
       call rhoIO(AL,vr_in_n(1,iislda),mr_n,2,fvr_in(iislda))    
       enddo
       ido_vr=0
       else
       ido_vr=1
       ...
       call rhoIO(AL,vext_n,mr_n,2,fvext_in)    
       else
       vext_n=0.d0
       endif
...
c        tim=rtc()
        ...
        if(ifl_trans.eq.1) then
       call Etranscalc(xatom,fatom,workr_n,Etot,
     &  iforce_cal,ido_rho,ido_vr,tolug,tolE,niter0,nline0,
     &  iCGmth0,iscfmth0,FermidE0,mCGbad0,E_st,err_st,AL,
...
******************** update some quantities
       Etot_old=Etot            ! the _old quantities correspond to xatom_old
...
**************************** store the ug and rho in file100xx
       ...
       call rhoIO(AL,rho_n(1,iislda),mr_n,1,f_tmp)    ! write down the charge density
************************************************************
...
**************************************************************
       enddo
*********************************************************
        call get_atomW(AL,xatom,xatom_w,ntype,iatom,
     &   totNel,islda)
...
994      format("  dt= ",E10.4,",  dd_max(a.u)=",E10.4,", 
     &     force_max=",E10.4)
****************************************************************
****************************************************************
...
******  We should not make rho_n positive here, that could lost local charge balance. 
        call move_rho(1,0)      ! iw=0, not use xatom_w

1000    continue      ! return point for more line_step for bad cases
     ...
     &  iCGmth1,iscfmth1,FermidE1,mCGbad1,E_st,err_st,AL,
     &  nkpt,ntype,convergE,islda,igga)
...
*****   the situations not to have a interpolation
        if(dabs(dt/dt_trial-1.d0).lt.0.0001.and.
     &      dabs(dd_max/dd_limit-1.d0).lt.0.0001) goto 5001

        interp=1
        if(dt/dt_trial.lt.0.15) interp=0
        if(dt/dt_trial.gt.4.d0) interp=0
        if(convergE.gt.100*tolE.and.convergE.gt.
     &      100*convergE_old) interp=0
        if(dd_max.gt.0.15) interp=0
        if(dabs(dt/dt_trial-1.d0).lt.0.02) interp=1


        if(inode.eq.1) then
        write(6,*) "**** inerpolation 1:yes, 0:no =", interp
        write(22,*) "**** inerpolation 1:yes, 0:no =", interp
        endif

       if(interp.eq.1) then
       call interpolation()
       else
       call move_rho(2,0)     ! ug_n not interpolated, but using the xatom_old values
       endif
****************************************************************
...
995      format(" **** E_prev,E_pred,E_final", 3x, 3(E18.12,1x))
997      format(" ****      dt= ", E9.3, ",   line_step= ",i3,
     &    ",  dt_incr_next_step= ", E9.3)
998      format(" ****  dd_max(a.u)= ", E9.3, ",   max_force= ",E9.3)
...
2001   continue     ! exit point
       ...
       close(23)
*******************************************************
       if(idens_out.eq.2)  then     ! output charge at the end of calculation
       if(kpt_dens.gt.nkpt.or.ispin_dens.gt.islda.or.
     &  iw_i.gt.iw_f.or.iw_f.gt.mx) then
       if(inode.eq.1) then
       write(6,*) "something wrong for output charge density"
       write(6,*) "kpt_dens.gt.nkpt.or.ispin_dens.gt.islda"
       write(6,*) "or.iw_i.gt.iw_f.or.iw_f.gt.mx, skip output charge"
       write(6,*) "kpt_dens,nkpt,ispin_dens,islda,iw_i,iw_f,mx", 
     &  kpt_dens,nkpt,ispin_dens,islda,iw_i,iw_f,mx 
       endif
       else
...
c103   format(5(f12.8,1x))
103   format(5(f18.14,1x))
...
*****************************************************
     ...
     &  MPI_DOUBLE_COMPLEX,0,100,MPI_COMM_WORLD,ierr)
            end if

            if (inode==1) then
              call mpi_recv(temp_array,mg_nx,
     &  MPI_DOUBLE_COMPLEX,i,100,MPI_COMM_WORLD,status,ierr)
...
280   continue
      ...
      endif      ! end of wavefunction output
...
***********************************************
       ...
       fz=0.d0
       do i=1,natom
cccccccc  note that fatom actually is the minus of the physical force
cccccccc  fatom is the energy direvitive which points to the increase of energy
...
******************************************************
       ...
       do iislda=1,islda
       call rhoIO(AL,vr_in_n(1,iislda),mr_n,1,fvr_out(iislda))   
       enddo
       endif

       if(irho_out.eq.1) then
       do iislda=1,islda
       call rhoIO(AL,rho_n(1,iislda),mr_n,1,frho_out(iislda))    
       enddo
       endif
...
*****************************************************
      ...
      ug_n(i,m)=ug_n_tmp(i,m)*fac1+ug_n(i,m)*fac2
      enddo
      enddo
      ...
      if(iislda.eq.1)  f_tmp="wrhofile1000"
      if(iislda.eq.2)  f_tmp="wrhofile2000"
      call rhoIO(AL,workr_n_tmp,mr_n,2,f_tmp)    

       do i=1,nr_n
       rho_n(i,iislda)=dabs(workr_n_tmp(i)*fac1+
     &                         rho_n(i,iislda)*fac2)
       ss=ss+rho_n(i,iislda)
       enddo
       call mpi_barrier(MPI_COMM_WORLD,ierr)
300    continue
       ...
       do iislda=1,islda
       do i=1,nr_n
       rho_n(i,iislda)=ss*rho_n(i,iislda)
       enddo
       enddo
...
*****  The resulting rho_n could be negative at some points, but that is okay to calc. potential. 

****** iw.eq.1, using xatom_w
****** iw.eq.0  using xatom_w=1, set xatom_w=1

        allocate(rhonew1_n(mr_n))

          if(iflag.eq.2) then
        f_tmp="wrhofile1000"
        call rhoIO(AL,rho_n(1,1),mr_n,2,f_tmp)    
        if(islda.eq.2) then   
        f_tmp="wrhofile2000"
        call rhoIO(AL,rho_n(1,2),mr_n,2,f_tmp)    
        endif
          endif

        if(iw.eq.1) then
        do isp=1,islda
        call getrho_only(AL,xatom_old,ntype,iatom,totNel,rhonew1_n,
     &   workr_n,xatom_w(1,isp))
        do i=1,nr_n
        rho_n(i,isp)=rho_n(i,isp)-rhonew1_n(i)
        enddo
        call getrho_only(AL,xatom,ntype,iatom,totNel,rhonew1_n,
     &    workr_n,xatom_w(1,isp))
        do i=1,nr_n
        rho_n(i,isp)=rho_n(i,isp)+rhonew1_n(i)
        enddo
...
cccc  using xatom_w makes the result worse ! 
cccc seems like, one atom always drag all the charge with it, thus overall it is neutral
     ...
     &   workr_n,xatom_w(1,1))
        do i=1,nr_n
        rho_n(i,1)=rho_n(i,1)-rhonew1_n(i)
        enddo
        call getrho_only(AL,xatom,ntype,iatom,totNel,rhonew1_n,
     &    workr_n,xatom_w(1,1))
        do i=1,nr_n
        ...
        if(islda.eq.2) then
        allocate(rhonew2_n(mr_n))
        call getrho_only(AL,xatom_old,ntype,iatom,totNel,rhonew1_n,
     &   workr_n,xatom_w(1,1))
        call getrho_only(AL,xatom,ntype,iatom,totNel,rhonew2_n,
     &    workr_n,xatom_w(1,1))
        do i=1,nr_n
        f1=rho_n(i,1)/(rho_n(i,1)+rho_n(i,2))
        f2=1.d0-f1
        rho_n(i,1)=rho_n(i,1)+(rhonew2_n(i)-rhonew1_n(i))*f1
        rho_n(i,2)=rho_n(i,2)+(rhonew2_n(i)-rhonew1_n(i))*f2
        enddo
        deallocate(rhonew2_n)
        endif

        endif
 

        deallocate(rhonew1_n)
