...
*********************************************
	  ...
	  k1=k3-1
       ...
       akkyz=0.5d0*(akky**2+akkz**2)
               
cc               if(akk.gt.Ecut) goto 5
               if(akkyz.gt.Ecut) goto 5
cccccccc special
...
cc                  if(akk.gt.Ecut) goto 15
                  if(akkyz.gt.Ecut) goto 15
...
cc                  if(akk.gt.Ecut) goto 200
                  if(akkyz.gt.Ecut) goto 200
...
cc                  if(akk.lt.Ecut*Smth) then
                  if(akkyz.lt.Ecut*Smth) then
                     wg_n(ig,kpt)=1.d0
                     ngc_in=ngc_in+1
                  else
c                     x=(akk-Ecut*Smth)/((1-Smth)*Ecut)*pi
                     x=(akkyz-Ecut*Smth)/((1-Smth)*Ecut)*pi
...
c     number of g vectors on my processor = ng_n
	 ...
	 if(ngtotnod(inode,kpt).gt.mg_nx) then
	 write(6,*) "ng_n.gt.mg_nx, stop", ngtotnod(inode,kpt),mg_nx
	 call mpi_abort(MPI_COMM_WORLD,ierr)
	 endif
