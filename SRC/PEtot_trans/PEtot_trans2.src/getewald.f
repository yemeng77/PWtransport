...
c      real*8 xatom(3,matom),fatom(3,matom)
      real*8 xatom(3,matom),fatom(3,matom),fatom_rec(3,matom)
      real*8 zatom(mtype)
...
c Modificado por Txomin      
      ...
      fatom(2,ia1)=0.d0   
      fatom(3,ia1)=0.d0
      fatom_rec(1,ia1)=0.d0
      fatom_rec(2,ia1)=0.d0
      fatom_rec(3,ia1)=0.d0
      ...
      if(ia2.ne.ia1) then
      derfrdr=(derfc(beta*(r+2.d-5))/(r+2.d-5)-
     &  derfc(beta*r)/r)/2.d-5
...
c Modificado por Txomin
      ...
      ewald_rec=ewald_rec+sr*ch1*ch2*0.5d0

c Modificado por Txomin
      fatom(1,ia1)=fatom(1,ia1)+srx*ch1*ch2
      fatom(2,ia1)=fatom(2,ia1)+sry*ch1*ch2
      fatom(3,ia1)=fatom(3,ia1)+srz*ch1*ch2
      fatom_rec(1,ia1)=fatom_rec(1,ia1)+srx*ch1*ch2
      fatom_rec(2,ia1)=fatom_rec(2,ia1)+sry*ch1*ch2
      fatom_rec(3,ia1)=fatom_rec(3,ia1)+srz*ch1*ch2
...
c Modificado por Txomin
      ...
      fatom_rec(1,ia1)=fatom_rec(1,ia1)+s_sin*gkx2_n(i)
      fatom_rec(2,ia1)=fatom_rec(2,ia1)+s_sin*gky2_n(i)
      fatom_rec(3,ia1)=fatom_rec(3,ia1)+s_sin*gkz2_n(i)

2002  continue

c Modificado por Txomin
      sq=sq+ff1*s_cos/gkk
      sq_rec=sq_rec+ff1*s_cos/gkk
...
c Modificado por Txomin
c      call mpi_allreduce(sq,sq,1,MPI_REAL8,
      call mpi_allreduce(sq,sq_rec,1,MPI_REAL8,
     & MPI_SUM,MPI_COMM_WORLD,ierr)
     	sq=sq_rec
c      call mpi_allreduce(ewald,ewald,1,MPI_REAL8,
      call mpi_allreduce(ewald,ewald_rec,1,MPI_REAL8,
     & MPI_SUM,MPI_COMM_WORLD,ierr)
     	ewald=ewald_rec
c      call mpi_allreduce(fatom,fatom,3*natom,MPI_REAL8,
      call mpi_allreduce(fatom,fatom_rec,3*natom,MPI_REAL8,
     & MPI_SUM,MPI_COMM_WORLD,ierr)
      fatom = fatom_rec
