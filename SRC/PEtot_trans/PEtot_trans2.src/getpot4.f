...
ccccccccc dirty trick to avoid the convergency
      if(dabs(rho(i)).gt.0.003) then
      vtot(i)=vtot(i)+vion(i)+vxuppbe+vcuppbe
      E_Hxc=E_Hxc+(expbe+ecpbe)*dabs(rho(i)+rhocr(i))
      else        ! use LDA formula
      vtot(i)=vtot(i)+vion(i)+UxcCA(dabs(rho(i)+rhocr(i)),uxc2)
      E_Hxc=E_Hxc+uxc2*dabs(rho(i)+rhocr(i))
      endif
