      subroutine rhoIO_comp_ym(AL,vr_n_tmp,mr_n,iflag,fdens_name,index)
******************************************
...
ccccccc iflag=2, read the charge density vr_n_tmp from fdens_name
      ...
      complex*16,allocatable,dimension(:)   :: tempvr1,tempvr2

      complex*16,allocatable,dimension(:)   :: temp_rho
    
      complex*16 vr_n_tmp(mr_n)
...
ccccccccccccccccccccccccccccccc
     ...
     &                 =tempvr2
               ...
               if (isend==0) then
                  ...
                  call mpi_send(tempvr1,nr/nnodes,
     &			MPI_DOUBLE_COMPLEX,isend,
...
cc            write(6,*)'iratio = ',iratio
     ...
     &                    102,MPI_COMM_WORLD,ierr)
         ...
         call mpi_recv(vr_n_tmp,nr/nnodes,
     &		MPI_DOUBLE_COMPLEX,0,102,
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      if(iflag.eq.1) then     ! write the charge 
       ...
       if(inode==i+1) then
        ...
        enddo
        call  mpi_send(temp_rho,mr_n,MPI_DOUBLE_COMPLEX,0,
     &   102,MPI_COMM_WORLD,ierr)

       endif

       if(inode==1) then
        call mpi_recv(temp_rho,mr_n,MPI_DOUBLE_COMPLEX,i,
