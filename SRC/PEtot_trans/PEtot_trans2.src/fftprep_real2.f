...
c
       call mpi_barrier(MPI_COMM_WORLD,ierr)
c Modificado por Txomin 
c       call mpi_allreduce(mgz2,mgz2,1,mpi_integer,mpi_max,
       call mpi_allreduce(mgz2,mgz2_rec,1,mpi_integer,
     &                    mpi_max,
     &                    MPI_COMM_WORLD, ierr)
       mgz2 = mgz2_rec
       call mpi_barrier(MPI_COMM_WORLD,ierr)
...
cccccc  safety check
      ...
      enddo
      if(isum.gt.nr1*nr2*nr3/nnodes) then
cccc WARNING
ccc  something might be wrong in the program, change nnodes can also remove the problem. 
      write(6,*) "sum ivpacn2l.gt.mr_n, increase n1,n2,n3,stop"
      call mpi_abort(MPI_COMM_WORLD,ierr)
      endif
